 ...
 */
function resolveCloudBaseCredentials(envId) {
    ...
    try {
        ...
        if (c.tmpExpired && Date.now() >= Number(c.tmpExpired)) {
            console.warn("[Config] tcb login credentials in ~/.config/.cloudbase/auth.json have expired; run `tcb login` to refresh.");
            return null;
        }
        return {
            envId,
        ...
        console.warn("[Config] Failed to read tcb login credentials:", err.message);
        return null;
    }
}
/**
 ...
 */
export function toKernelAgentConfig(config, opts = {}) {
    ...
    for (const [name, policy] of builtinPolicies) {
        if (policy.enabled && policy.permissionPolicy.type === "always_ask") {
            approvalNames.push(isLocalProvider
    ...
    for (const t of getMcpToolsets(config)) {
        ...
        if (defaultAsk)
            approvalNames.push(`mcp__${t.mcp_server_name}__*`);
    ...
    const allApprovalNames = Array.from(new Set([...approvalNames, ...customToolNames]));
    const effectiveRequireApproval = allApprovalNames.length > 0 ? allApprovalNames : undefined;
    // ── Credentials ─────────────────────────────────────────────────────────
    ...
    const effectiveCredentials = credentials ??
        (hasApiKey
            ...
            : null);
    // ── Model spec ──────────────────────────────────────────────────────────
    const baseModel = typeof config.model === "string"
        ? { id: config.model }
        : { ...config.model };
    const model = baseModel.apiKey || baseModel.apiBaseUrl || baseModel.options
        ? baseModel
        : baseModel.id;
    // ── Sandbox ─────────────────────────────────────────────────────────────
    ...
    return {
        ...
        tools: opts.customToolDefs && opts.customToolDefs.length > 0
            ? opts.customToolDefs
            : undefined,
        skills: skillNames.length > 0 ? { enabled: skillNames } : undefined,
    };
}
