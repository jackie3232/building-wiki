...
// ── Singletons ───────────────────────────────────────────────────────────────
let _kernelAgent = null;
let _sessionStore = null;
/** Build (or return cached) kernel Agent for this process. */
export function getKernelAgent(config) {
    if (_kernelAgent)
        return _kernelAgent;
    ...
    _sessionStore = kernelConfig.session?.store ?? null;
    console.log(`[KernelAdapter] sessionStore captured: type=${typeof _sessionStore}, ` +
 ...
 */
export async function syncRegisterSession(sessionId, userId, title) {
    const ts = Date.now();
    if (!_sessionStore?.registerSession) {
        const reason = `_sessionStore=${_sessionStore === null ? "null" : typeof _sessionStore}, ` +
 ...
 */
export async function getOrCreateKernelSession(config, acpSessionId, opts = {}) {
...
export function dropKernelSession(acpSessionId) {
    sessionPool.delete(acpSessionId);
}
export async function abortKernelSession(acpSessionId) {
...
export function registerKernelSession(acpSessionId, session) {
    sessionPool.set(acpSessionId, session);
}
 ...
 */
export async function pumpEvents(events, ctx) {
    for await (const item of events) {
        ...
        const params = { ...(raw.params ?? {}), sessionId: ctx.acpSessionId };
...
export function makeSseSink(res) {
...
export function makeClientSideToolDefinition(tool) {
    ...
    return {
        ...
        execute: async (_input, _ctx) => {
            return (`[oak-runtime] Tool '${tool.name}' is declared as client-side in agent.yaml ` +
