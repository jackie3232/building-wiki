...
import { abortKernelSession, dropKernelSession, getKernelAgent, getOrCreateKernelSession, makeSseSink, outcomeToDecision, pumpEvents, registerKernelSession, syncRegisterSession, } from "./kernel-adapter.js";
import { withActiveSpan } from "open-managed-agent-runtime-shared/telemetry.js";
import { rpcResult, rpcError, sseStart, sseWrite, sseDone, sseSessionUpdate, isAcpUuid, } from "open-managed-agent-runtime-shared/acp-shared.js";
// ── Helpers ──────────────────────────────────────────────────────────────────
...
function recordToAcpMessage(rec) {
    const parts = [];
    let textBuf = "";
...
// ── RPC handlers ─────────────────────────────────────────────────────────────
function handleInitialize(_params, config) {
    return {
        protocolVersion: 1,
        agentCapabilities: {
            loadSession: true,
...
async function handleSessionNew(params, config, req) {
    const reqSessionId = params.conversationId ??
        params.sessionId;
    const meta = params.meta ?? {};
    const title = meta.title ?? undefined;
    const userId = resolveUserId(req, meta.userId ?? "anonymous");
    ...
    if (reqSessionId) {
        if (!isAcpUuid(reqSessionId)) {
            throw Object.assign(new Error(`Invalid sessionId: must be a UUID (got "${reqSessionId.slice(0, 64)}"). ` +
                `Use crypto.randomUUID() to generate one client-side, or omit the field ` +
                `to let the server generate it.`), { rpcCode: -32602 });
        }
        const existing = await agent.sessions.get(reqSessionId);
        if (existing)
            return { sessionId: reqSessionId, hasHistory: true };
...
async function handleSessionList(params, config) {
    const agent = getKernelAgent(config);
...
async function handleSessionLoad(params, res, id, config) {
...
async function handleSessionPrompt(params, res, id, config) {
    ...
    try {
        ...
        if (toolResultBlock) {
            ...
            const outcome = permissionBlock.decision === "cancelled"
                ? { outcome: "cancelled" }
...
async function handleSessionDelete(params, config) {
    const sessionId = String(params.sessionId ?? "");
    if (!sessionId)
...
async function handleSessionResume(params, config) {
    const sessionId = String(params.sessionId ?? "");
    if (!sessionId)
        throw new Error("sessionId is required");
    const agent = getKernelAgent(config);
...
// ── Mount function ──────────────────────────────────────────────────────────
export function mountManagedAcpEndpoint(app, agentConfig) {
    ...
    const acpHandler = async (req, res) => {
        const body = req.body;
        ...
        try {
            switch (method) {
                case "initialize":
                    return res.json(rpcResult(id, await withActiveSpan("managed.initialize", {}, async () => handleInitialize(params, agentConfig))));
                case "session/new":
                    return res.json(rpcResult(id, await withActiveSpan("managed.session.new", { sessionId: String(params.sessionId ?? params.conversationId ?? "") }, async () => handleSessionNew(params, agentConfig, req))));
                case "session/list":
                    return res.json(rpcResult(id, await withActiveSpan("managed.session.list", {}, async () => handleSessionList(params, agentConfig))));
                case "session/load":
                    await withActiveSpan("managed.session.load", { sessionId: String(params.sessionId ?? "") }, async () => { await handleSessionLoad(params, res, id, agentConfig); });
                    return;
                case "session/prompt":
                    await withActiveSpan("managed.session.prompt", { sessionId: String(params.sessionId ?? "") }, async () => { await handleSessionPrompt(params, res, id, agentConfig); });
                    return;
                case "session/cancel":
                    await withActiveSpan("managed.session.cancel", { sessionId: String(params.sessionId ?? "") }, async () => { await handleSessionCancel(params); });
                    if (isNotification)
                        return res.status(204).end();
                    return res.json(rpcResult(id, { ok: true }));
                case "session/delete":
                    return res.json(rpcResult(id, await withActiveSpan("managed.session.delete", { sessionId: String(params.sessionId ?? "") }, async () => handleSessionDelete(params, agentConfig))));
                case "session/resume":
                    return res.json(rpcResult(id, await withActiveSpan("managed.session.resume", { sessionId: String(params.sessionId ?? "") }, async () => handleSessionResume(params, agentConfig))));
                default:
                    if (isNotification)
