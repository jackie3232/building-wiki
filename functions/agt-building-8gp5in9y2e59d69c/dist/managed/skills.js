...
const BUNDLE_SKILLS_DIR_CANDIDATES = ["/var/user/skills", "/app/skills"];
...
export function oakSkillDestPath(workspaceCwd, skillName) {
    return path.join(workspaceCwd, ".claude", "skills", skillName, "SKILL.md");
}
function bundleSkillsDirCandidates(opts) {
    const cwd = opts.cwd ?? process.cwd();
    const runtimePkgRoot = opts.runtimePkgRoot ??
 ...
 */
export function resolveBundleSkillsDirSync(opts = {}) {
    const runtimePkgRoot = opts.runtimePkgRoot ??
        path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "..");
...
export async function resolveBundleSkillsDir(opts = {}) {
    return resolveBundleSkillsDirSync(opts);
}
/** Skill directory names present in the deploy bundle (sync). */
export function listBundledSkillNames(bundleSkillsDir) {
...
async function isSkillDirectory(dir) {
...
async function materializeLegacyFlatFile(bundleSkillsDir, name, destDir) {
 ...
 */
export async function materializeManagedSkills(skillNames, opts = {}) {
    ...
    if (!(await pathExists(bundleSkillsDir))) {
        ...
        console.warn(`[ManagedSkills] Bundle skills dir not found: ${bundleSkillsDir}`);
    ...
    for (const rawName of skillNames) {
        ...
        try {
            ...
            console.warn(`[ManagedSkills] Skill '${name}' not found under ${bundleSkillsDir}`);
        }
        catch (err) {
            skipped.push(name);
            wl.error(err, { phase: "materialize_skill", skill: name });
            console.warn(`[ManagedSkills] Failed to materialize '${name}': ${err.message}`);
        }
    }
    wl.emit({
